...
import { ShieldCheck, TrendingUp, Eye, Briefcase } from 'lucide-react';
// Note: Adjust these import paths if your components are in a different folder
...
export default function SellerDashboard() {
  ...
  useEffect(() => {
    const getData = async () => {
      ...
      // Placeholder for data fetching - ensure this matches your real data structure
      // For now, we initialize safe defaults to prevent build crashes
      if (user) {
         setData({
            totalActiveListings: 0,
            listingPerformance: [],
  ...
  // Safe calculation of stats using reduce
  const totalViews = data.listingPerformance?.reduce((acc: number, curr: any) => acc + (curr.views || 0), 0) || 0;
  const qualifiedBuyers = data.listingPerformance?.reduce((acc: number, curr: any) => acc + (curr.qualifiedBuyers || 0), 0) || 0;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-8 relative z-10">
        ...
        <section className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden min-h-[200px]">
           ...
           <div className="p-8 text-center text-slate-500">
              {data.totalActiveListings === 0 ? "You have no active listings." : "Loading listings..."}
           </div>
        </section>
