...
export default async function BrowseOperationalPage({
  searchParams,
}: {
  searchParams: { [key: string]: string | string[] | undefined };
}) {
  // Track is fixed to 'operational'
  const assetTypeFilter = 'Operational';
  const sp = searchParams || {};

  // --- START TRANSLATION LAYER ---
  // 1. Asset Type is already set to 'Operational'
  // 2. Normalize Prices
    ...
    <div className="flex flex-col min-h-screen pt-20 bg-slate-50/50">
      <div className="max-w-[1600px] mx-auto w-full px-6 py-8">
        <div className="mb-8 border-b pb-6">
          <h1 className="text-3xl font-bold text-slate-900">🏢 Operational Businesses</h1>
          ...
          {isUnknownCategory && categoryCode && (
            <div className="mt-4 p-4 bg-amber-50 border border-amber-200 rounded-lg">
              <p className="text-sm text-amber-800">
                ...
                Showing all operational listings instead.
              </p>
            </div>
          )}
